...
function leaveGame() {
  if (botinfo.connected) {
    setUserVal("", {});
    botinfo.fbdb = false;
    // Use the correct Firebase SDK method to delete the app
    if (botinfo.liveApp) {
      botinfo.liveApp.delete();
    }
    botinfo.connected = false;
...
function errorBar(text) {
  ...
  if (errorElement) {
    ...
    errorElement.classList.remove("errorBar-initial");
    errorElement.classList.add("errorBar-active");

    // Auto-hide after 5 seconds
    setTimeout(function () {
      errorElement.classList.remove("errorBar-active");
      errorElement.classList.add("errorBar-initial");
    }, 5000);
  } else {
...
async function connect(gid, name, icog, reqbody = !1) {
  ...
  if (body.success) {
    ...
    await firebase.auth(liveApp).signInWithCustomToken(body.fbToken);
...
// simple online count code (DISABLED - endpoint not implemented)
